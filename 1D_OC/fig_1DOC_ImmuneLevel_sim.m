% Figure:  1DOC with an input immune level 
...
% **Note**: Given initla resistant bacteria R(0) = 1, single type of phage
%         control fails immediately, no matter how to balance the regulator
%         weights. When immune-level >= 9e6, 1D-OC works.
% Note: Io > 1.2e7, immune can kill bacteria alone.
...
switch sim_case
    case 1 % High immune basal level parameters
        ...
        weight_theta2 = 1e-11; % weight of regularization of injection 
        Name = 'fig_1DOC_immune_high';
    case 2 % Low immune basal level parameters
        Io = 3e6; % initial immune level
        weight_theta2 = 1e-11; % read from text.file
        Name = 'fig_1DOC_immune_low';
    case 3 % High immune basal level parameters
        Io = 8.5e6; % initial immune level
        weight_theta2 = 1e11; % weight of regularization of injection 
        Name = 'fig_1DOC_immune_very_high';
end 
...
%% Fixed parameters 
delay_hours = 2; % fixed
p.P_in = 1e9; % maximal injection rate of control, fixed
...
kk = 0;  N_iter = 30; tol_eps = 1e-10; % total iterations and convergence tolerance
...
if sim_case == 3
    ...
    yticks([1e-2 1 1e2 1e4 1e6 1e8 1e10]); 
else
    axis([0,max(t),1e-2,1e12]); 
    yticks([1e-2 1 1e2 1e4 1e6 1e8 1e10 1e12]); 
end
...
if isempty(index_be) 
    ...
    patch([min(xlim) max(xlim) max(xlim) min(xlim)], [ybars(1) ybars(1), ybars(2) ybars(2)],...
        [0.6 0.6 0.6], 'FaceAlpha',.5, 'linestyle','none');

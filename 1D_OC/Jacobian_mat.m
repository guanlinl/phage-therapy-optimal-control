function dfdx = Jacobian_mat(x1,x2,x3,x4,p)
dfdx = zeros(4,4);
...
% construct Jacobian matrix dfdx, size 4 by 4
% row 1
dfdx(1,1) = ((1 - m)*r*(Kcd0 - 2*x1 - x2)/Kcd0) - ...
    (Kpd0*F_P) - (ep0*(1 + x2)*x4/(1 + x1 + x2)^2);
...
dfdx(2,2) = -m*r*x1/Kcd0 + rp*(Kcd0 - x1 - 2*x2)/Kcd0 - ...
    ep0*(1 + x1)*x4/((1 + x1 + x2)^2);
dfdx(2,4) = -ep0*x2/(1 + x1 + x2);

% row 3
dfdx(3,1) = beta*F_P - phi0*x3;

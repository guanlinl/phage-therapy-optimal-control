function result = treatment_2D_OC(therapy_scenario,immune_level,weight_theta2, FinalTime)
...
p.t0 = delay_hours; p.tf = FinalTime; % for numerical purpose
...
switch initial_guess
    ...
    case 2 
      u0 = 0.1.*ones(p.N,2); % uniformly = 0.5
...
N_iter = 50; tol_eps = 1e-6; % total iterations
...
us = u(:,1); ur = u(:,2);
t_ps_index = find(us == max(us)); t_pr_index = find(ur == max(ur));
t_ps = t(t_ps_index); t_pr = t(t_pr_index);
...
if isempty(find(N_sum < 1)) == 0
    result = [1, V_P, V_PS, V_PR, t_ps, t_pr]; % phage control works
else
    result = [0, 0, 0, 0, 0, 0]; % fail
end

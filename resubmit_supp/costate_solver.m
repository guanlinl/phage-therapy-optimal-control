...
lam_sol = zeros(5,N); % initialization
lam_terminal = [p.theta2;p.theta2;0;0;0]; 
lam_sol(:,N) = lam_terminal; % terminal condition
state_sol = state_sol(:,2:6); state_sol = state_sol';
%% -------------------- Numerical solvers ---------------------
 % backward euler method 
        for i = 1:N-1
            f_adj = adjODE(state_sol(:,N + 1 - i),lam_sol(:,N + 1 - i),...
                p); % dlambda/dt

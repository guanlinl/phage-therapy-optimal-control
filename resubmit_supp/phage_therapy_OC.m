% Package-like function for 2D control
% Input:  claim which region, immune eff or Myd88, therapy_scenario {1,2}
%         immune_level: if therapy_scenario = 1, immune_level is fixed
%         the maximal injection rate

% Output: [0, 0] if control doesn't work
%         [1, total_dosage] if control works

% Note: we don't output population dynamics and controls, refer to 
%       control_test.m file for the simulations.

function result = phage_therapy_OC(therapy_scenario,immune_level,max_inj,...
    delay_hours)
%% ----------------------- Model parameters -----------------------
% Fixed parameters 
...
switch therapy_scenario
    case 1 % wild-type
        ...
        % initial densities
        So = 7.4e7; Ro = 0; Po = 0; Io = 2.7e6; P2o = 0; 
    case 2 % Myd88
        % total bacteria carrying capacity
        p.Kc = 8.5e11; 
        p.P_in = max_inj;
        % initial densities
        So = 7.4e5; Ro = 0; Po = 0; P2o = 0; Io = immune_level;
...
p.theta0 = 1e2; p.theta1 = 1e-2; p.theta2 = 1; % weights in cost functional
...
kk = 0;  N_iter = 15; % total iterations

...
% (S,R,P,I,P2) -> (rescale) -> (x1,x2,x3,x4,x5)
dxdt = [0;0;0;0;0]; % initialization
...
dxdt(1) = r*x(1)*(1 - n/Kcd0)*(1-m) - Kpd0*x(1)*F_P1 - ep0*x(1)*x(4)/(1 + n);
dxdt(2) = rp*x(2)*(1 - n/Kcd0) + r*x(1)*(1 - n/Kcd0)*m - Kpd0*x(2)*F_P2 - ...
    ep0*x(2)*x(4)/(1 + n);
dxdt(3) = (beta*x(1)*F_P1) - phi0*x(1)*x(3) - w*x(3) + q0*u(1);
dxdt(4) = a*x(4)*(1 - x(4))*(n/(n + Knd0));
dxdt(5) = (beta*x(2)*F_P2) - phi0*x(2)*x(5) - w*x(5) + q0*u(2);

% numerical correction.
for k = 1:5
     if x(k) + dxdt(k)*p.dt < 0
         dxdt(k) = 0;
     end
end

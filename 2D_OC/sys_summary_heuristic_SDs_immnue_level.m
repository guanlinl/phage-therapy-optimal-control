...
pratical_dosage_2D_SDs = ...
    importdata('pratical_dosage_2D_SDs_immnue_level.txt');
amp_fac_seq = pratical_dosage_2D_SDs(:,2);

% Savers
time_S = zeros(1,length(immune_range_2d));
time_R = zeros(1,length(immune_range_2d));
dosage_S = zeros(1,length(immune_range_2d));
dosage_R = zeros(1,length(immune_range_2d));


for ii = 1:length(immune_range_2d)
% parameter of interests
Io = immune_range_2d(ii); 
weight_theta2 = theta2_2D_OC(ii);
amp_fac = amp_fac_seq(ii); 
...
time_S(ii) = time_points(3); time_R(ii) = time_points(4);
dosage_S(ii) = V_PS*(1 + amp_fac); dosage_R(ii) = V_PR*(1 + amp_fac);
...
% write data file
y = [immune_range_2d'; time_S;  time_R; dosage_S; dosage_R];
fileID = fopen('data_summary_heuristic_SDs_immune_level.txt','w');
